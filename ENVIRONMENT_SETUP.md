...
- `FRONTEND_ORIGIN`: **Required in production**. The frontend URL (e.g., `https://qtma25.vercel.app`)
...
- **Requires** `FRONTEND_ORIGIN` to be set (e.g., `https://qtma25.vercel.app`)
...
1. **Backend `.env` file:**
   ```bash
   ENVIRONMENT=dev
   FRONTEND_ORIGIN=https://qtma25.vercel.app  # Optional, not used in dev mode
...
1. **Backend `.env` file (on Render/Vercel/etc):**
   ...
   FRONTEND_ORIGIN=https://qtma25.vercel.app  # REQUIRED
   # ... other required variables
   ```
...
3. **Supabase Redirect URLs**: Make sure both localhost and production callback URLs are registered in Supabase:
   ...
   - `https://qtma25.vercel.app/auth/callback` (for prod)
